...
def construct_feed_dict(features, weights, labels, labels_mask, placeholders):
    """Construct feed dictionary."""
    feed_dict = dict()
    feed_dict.update({placeholders['weights'][i]: weights[i] for i in range(len(weights))})
...
def partial_mask(adj):
    adj_np_array = np.array(adj.todense(), dtype=np.float32)
    return adj_np_array
...
def prune_adj2(oriadj, non_zero_idx, percent):
    ...
    low_pcen = np.percentile(abs(non_zero_low_adj), percent, interpolation='lower')
